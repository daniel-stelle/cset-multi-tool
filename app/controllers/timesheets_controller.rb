class TimesheetsController < ApplicationController
  ...
  def clock_in
    ...
    if !@ts_rows.any? || clocked_out?(@ts_rows.last)
      @ts.timesheet_rows.build(date_worked: Date.today, time_in: Time.zone.now)
      @ts.save
    else
      flash[:danger] = "You must clock out first."
    end
    
    redirect_to user_timesheet_path(id: @ts.id, user_id: @ts.user_id)
  end
  ...
  def clock_out
    ...
    # See about putting checks into model
    unless !@ts.timesheet_rows.any? || @ts.timesheet_rows.last.time_out? 
  ...
  private
    ...
    def timesheet_params
      params.require(:timesheet).permit(:role_id, timesheet_row_attributes: [:id, :date_worked, :time_in, :time_out])
    end
end
